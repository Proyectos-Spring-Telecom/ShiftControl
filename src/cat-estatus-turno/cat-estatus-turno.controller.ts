...
@ApiTags('Cat Estatus Turno')
@ApiBearerAuth('bearer-token')
@UseGuards(JwtAuthGuard, RolesGuard)
@Roles()
@Controller('cat-estatus-turno')
export class CatEstatusTurnoController {
  constructor(private readonly service: CatEstatusTurnoService) { }

  @Post()
  @Roles(1)
  @ApiOperation({
    summary: 'Crear estatus de turno',
    description: 'Crea un registro con estatus activo (1). Solo SuperAdministrador.',
  })
  @ApiBody({ type: CreateCatEstatusTurnoDto })
  @ApiResponse({ status: 201, description: 'Creado correctamente' })
  @ApiResponse({ status: 400, description: 'Datos inválidos' })
  @ApiResponse({ status: 401, description: 'No autorizado' })
  @ApiResponse({ status: 403, description: 'Acceso denegado' })
  async create(
    @Body() dto: CreateCatEstatusTurnoDto,
    @Request() req,
  ...
  @Get('list')
  @ApiOperation({
    summary: 'Lista de estatus de turno activos',
  ...
  @ApiParam({ name: 'id', type: 'number', example: 1 })
  @ApiBody({ type: UpdateCatEstatusTurnoEstatusDto })
  ...
  async updateEstatus(
    ...
    @Body() dto: UpdateCatEstatusTurnoEstatusDto,
    @Request() req,
  ): Promise<ApiCrudResponse> {
    const idUser = req.user.userId;
  ...
  @Patch(':id')
  @ApiOperation({ summary: 'Actualizar estatus de turno' })
  @ApiParam({ name: 'id', type: 'number', example: 1 })
  @ApiBody({ type: UpdateCatEstatusTurnoDto })
  ...
  async update(
    @Param('id', ParseIntPipe) id: number,
    @Body() dto: UpdateCatEstatusTurnoDto,
    @Request() req,
  ): Promise<ApiCrudResponse> {
    const idUser = req.user.userId;
