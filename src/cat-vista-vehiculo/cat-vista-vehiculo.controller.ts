...
@ApiTags('Cat Vista Vehiculo')
@ApiBearerAuth('bearer-token')
@UseGuards(JwtAuthGuard, RolesGuard)
@Roles()
@Controller('cat-vista-vehiculo')
export class CatVistaVehiculoController {
  constructor(private readonly service: CatVistaVehiculoService) { }

  @Post()
  @Roles(1)
  @ApiOperation({
    summary: 'Crear vista del vehículo',
    description: 'Crea un registro con estatus activo (1). Solo SuperAdministrador.',
  })
  @ApiBody({ type: CreateCatVistaVehiculoDto })
  @ApiResponse({ status: 201, description: 'Creado correctamente' })
  @ApiResponse({ status: 400, description: 'Datos inválidos' })
  @ApiResponse({ status: 401, description: 'No autorizado' })
  @ApiResponse({ status: 403, description: 'Acceso denegado' })
  async create(
    @Body() dto: CreateCatVistaVehiculoDto,
    @Request() req,
  ...
  @Get('list')
  @ApiOperation({
    summary: 'Lista de vistas del vehículo activas',
  ...
  @ApiParam({ name: 'id', type: 'number', example: 1 })
  @ApiBody({ type: UpdateCatVistaVehiculoEstatusDto })
  ...
  async updateEstatus(
    ...
    @Body() dto: UpdateCatVistaVehiculoEstatusDto,
    @Request() req,
  ): Promise<ApiCrudResponse> {
    const idUser = req.user.userId;
  ...
  @Patch(':id')
  @ApiOperation({ summary: 'Actualizar vista del vehículo' })
  @ApiParam({ name: 'id', type: 'number', example: 1 })
  @ApiBody({ type: UpdateCatVistaVehiculoDto })
  ...
  async update(
    @Param('id', ParseIntPipe) id: number,
    @Body() dto: UpdateCatVistaVehiculoDto,
    @Request() req,
  ): Promise<ApiCrudResponse> {
    const idUser = req.user.userId;
